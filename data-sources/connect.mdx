---
title: "Connect"
description: "Authorize Oviond to pull data from Google Ads, Facebook, and 80+ other platforms."
---
...
You manage data sources in three places:

- **A client's Data sources page** — open the client and click **Data sources** in the sidebar. Every platform is listed as a card, connected ones first. This is the primary place to wire up data, and it works before the client has a single report. See [Client Data Sources](/data-sources/client-sources).
- **A report's Data Sources tab** — open a report, then click **Data Sources** in the editor sidebar. The same flow, scoped to that report's client, for when you're already building.
- **Settings → Data sources** (`/settings/data-sources`) — a workspace-wide table of every connection. Use it to rename, refresh, or delete connections and to see which clients use each one.
...
<Steps>
  <Step title="Find the connection">
    Open the client's **Data sources** page (or **Settings → Data sources**) and look for a connection showing **Token Expired**.
  </Step>
  <Step title="Refresh">
    Click **Refresh** (on the connection) or **Refresh token** (in Settings). Oviond reopens the OAuth popup.
  </Step>
...
<AccordionGroup>
  <Accordion title="Wrong account selected during OAuth">
    ...
    2. Start **Connect** again from the client's **Data sources** page.
