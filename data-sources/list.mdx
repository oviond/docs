...
Oviond supports 80+ marketing platforms across categories such as paid search, paid social, analytics, SEO, email marketing, e-commerce/CRM, and databases.
...
| **Email Marketing** | Mailchimp, Klaviyo, Omnisend, Campaign Monitor, ActiveCampaign, Vbout |
| **E-commerce & CRM** | Shopify, WooCommerce, HubSpot, Salesforce, Pipedrive, Stripe |
| **Databases & spreadsheets** | Airtable, Google Sheets |

<Note>
  The full list of available data sources appears on a client's [Data sources](/data-sources/client-sources) page, and in the report editor's **Data Sources** tab. New platforms are added regularly.
</Note>
...
To see the data sources linked to one client, open that client and click **Data sources** in the sidebar. Platforms the client is already connected to sort to the top, each card reading **Connected**.

## Checking connection status

Oviond checks token health automatically with a daily auth-test. You don't run a manual test — each connection displays a **Token valid** or **Token expired** badge. When a token expires, click **Refresh** (on the connection) or **Refresh token** (Settings) to re-authorize. See [Test Connection](/data-sources/test) for details.
...
1. Open the client and go to its **Data sources** page.
