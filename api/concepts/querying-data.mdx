...
The valid `metrics`, `dimensions`, and `data_view` values differ per datasource. Discover them from the datasource's **field catalog**:

```bash
# Metrics — use the returned `id`s as the `metrics` array
curl "https://api.oviond.com/v1/data/gadw/metrics" \
  -H "Authorization: Bearer YOUR_API_KEY"

# Dimensions — use the returned `id`s as the `dimensions` array
curl "https://api.oviond.com/v1/data/gadw/dimensions" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

Both group results by data view; add `?data_view=ACCOUNT` to filter to one view. Each entry has an `id` (what you pass to the query) and a human-readable `name`.

Other helpful endpoints:

- **`GET /v1/clients/{id}/datasources`** — which sources a client has connected (and their `datasource_id`s).
- **`POST /v1/data/resource`** — selectable sub-resource values (campaigns, ad sets, videos) for filters.

<Tip>
  Over MCP, the `data_list_metrics` and `data_list_dimensions` tools return the same catalog — an agent calls those first, then `data_query`.
</Tip>
