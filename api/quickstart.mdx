...
Every report belongs to a client.
...
Data-source connections require interactive OAuth, so connect them once in the app — open the client's **Data sources** page and pick a platform (see [Connecting Data Sources](/data-sources/connect)). Once connected, confirm it over the API:
...
```bash
curl -X POST "$OVIOND_BASE/v1/reports" \
  -H "Authorization: Bearer $OVIOND_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Q1 Report",
...
Use `"type": "DASHBOARD"` for a live dashboard instead of a paged, point-in-time report. The response returns the new report's `id` and `nano_id`. See the [Create Report reference](/api/introduction) for the full set of layout and branding fields.
...
- **Share a link:** open the report in the app and use the **Share** dialog to publish a link (backed by the report's `nano_id`).
