...
Permanently deleting an archived item is **irreversible** — the row is removed from the database along with all its children (a client's reports and widgets, a report's widgets and pages, a template's pages and widgets, and so on). For media, the underlying file is also removed from storage.
