...
The top-level **Automations** screen lists every automation in your account. Each row shows the automation's name, report, schedule, next send, and a status badge. Use the search box to filter by name, report, recipient, template, or sender, and the filter tiles to narrow the list.
...
| **Executions** | Opens the run history for the report — see [Run history](/automations/history). |
