...
Archiving an automation stops it from running and removes it from the active **Automations** list. It's a soft delete — the configuration is retained, so you can restore the automation from the archive later. The underlying **report** is not affected; only the scheduled delivery stops.

<Steps>
  <Step title="Open the automation">
    From the **Automations** screen, use the row menu or open the details slide-over. You can also archive from the **Automations** tab inside the report editor.
  </Step>
