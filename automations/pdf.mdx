...
Oviond renders a **Multi-Page Report** as a PDF on demand. Generation runs asynchronously in the background, so large reports don't block your browser — Oviond queues a job, renders the report headlessly, and stores the finished file for download.

<Note>
PDF export is for Multi-Page Reports only. A Dashboard is a single continuous view with no page boundaries, so there's nothing to paginate — dashboards show no **Download** tab and no **Allow PDF Download** option.
</Note>
...
<Steps>
  <Step title="Open the share dialog">
    From the report, open **Share** and switch to the **Download** tab.
  </Step>
...
1. `POST /v1/pdf/generate` validates the request and enqueues the job, returning a `job_id`. The request specifies the `report_id`, `client_id`, a page `format` (**A4** or **Letter**), and `orientation` (**portrait** or **landscape**). The defaults are `A4` and `portrait`.
2. `GET /v1/pdf/status/:job_id` is polled until `status` is `complete` (which returns the file `url`) or `failed`.

Each attempt is recorded in the report's export history, which the **Download History** list reads from.
