...
Configure white-label settings once to establish your agency's identity across all shared reports. Use branding settings to define the visual style of report content — either as a default for all reports or as a baseline your per-report themes build on. See [White-Label Reporting for Your Agency](/white-label/overview) for the account-level identity settings.
...
If a project has a theme applied, that theme's colors and fonts take effect for that project's client-facing view. Projects without an applied theme fall back to your branding settings. See [Design Report Themes](/reports/themes) for how to create and apply per-report themes.
