---
title: "Archive client"
description: "Soft-delete a client and restore it later from the Archive."
---

Archiving a client removes it from your active dashboard without permanently destroying anything. The client's reports, pages, widgets, and data connections are preserved and can be restored at any time.

## Archive a client

You can archive a single client from its settings, or several at once from the Clients list.

<Steps>
  <Step title="Open the client's settings">
    Open the client and go to its **Settings**, then scroll to the **Danger zone**.
  </Step>
  <Step title="Archive the client">
    Click **Archive client**. Oviond explains that the client moves to the archive and reports stay attached.
  </Step>
  <Step title="Confirm">
    Confirm with **Archive client**. The client moves to the [Archive](/archive/view) and disappears from the active list.
  </Step>
</Steps>

To archive multiple clients at once, select them on the Clients list and click **Archive** in the action bar. See [Bulk archive](/clients/bulk-delete).

## Restore an archived client

Go to **Settings → [Archive](/archive/view)**, find the client, and click **Restore**. The client returns to your active list with all its reports intact.

## Permanent deletion

Archived clients stay in the Archive until you permanently delete them. From the Archive, select a client and click **Delete** to remove it permanently. Permanent deletion is unrecoverable, so export anything you need to keep first.
