---
title: "Authenticate with the Oviond API"
...
```bash
curl -X GET https://backend.oviond.com/v1/users/me \
...
```bash
curl https://backend.oviond.com/v1/users/me \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
You can list, create, and revoke API keys at any time from **Settings → API Keys** in the app, or via the [API Keys API](/api-keys/overview).
