...
**Tools:** `list_assets` (find the section) → `list_pages` → `add_asset_to_page`
**What happens:** the agent finds the saved section with `list_assets` (search by name), reads the report's pages with `list_pages`, then `add_asset_to_page` clones the section's widgets onto the page — each with a fresh id, stacked below what is already there.
