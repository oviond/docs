...
A **report** is what you build inside the client — a multi-page report or a dashboard. See [Add Report](/reports/add).
...
<Steps>
  ...
  <Step title="Add a report">
    On the client's **Reports** tab, click **Add Report**.
  </Step>
  ...
  <Step title="Pick the type">
    Choose **Multi-Page Report** (a paged report for a specific date range) for your first build. **Dashboard** is a live, single-screen view.
  </Step>
  <Step title="Name and create">
    Name it something like *Acme — Monthly Performance*, then click **Create report**. Oviond opens the report editor.
  </Step>
</Steps>

## 3. Connect a data source

You wire up data from the client's **Data sources** page, or inside the report editor in its **Data sources** tab. See [Connect](/data-sources/connect).
...
<Note>
Oviond auto-saves the editor as you work and saves the whole report — every page and widget — in one operation. See [Add a widget](/widgets/add).
</Note>
...
<Steps>
  ...
  <Step title="Apply it to the report">
...
See [Share Report](/reports/share). Click **Share** in the top-right of the editor to open the Share dialog.
