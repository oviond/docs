...
- You have a [report](/reports/add) — multi-page report or dashboard — ready to send.
...
- You've tested the report's widgets with live data.
...
<Steps>
  ...
  <Step title="Preview a PDF">
    Open the report's **Share** dialog, switch to the **Download** tab, and click **Generate New PDF**. Generation runs in the background; check that the rendered report looks clean. See [PDF export](/automations/pdf).
  </Step>
</Steps>

## 2. Create the automation

You can create an automation from the top-level **Automations** screen or from the report editor's **Automations** tab. See [Add an automation](/automations/add).

<Steps>
  <Step title="Open Automations">
    Click **Automations** in the left sidebar, then click **New automation**. (From the report editor's **Automations** tab, click **Add automation** instead, and the report is pre-selected.)
  </Step>
  ...
  <Step title="Pick the report and date range">
    Choose the **Client** and **Report** you validated, then set **Date range** to **Last Month**. The report is rendered fresh for that range on every run.
  </Step>
  <Step title="Configure the email">
    Add the client contact under **Recipients** (you can add several). Choose the **From** sender, and pick an **Email template** — or choose **Custom message** to compose a **Subject** and **Message** inline. Placeholders like the client name and date range fill in from the report. See [Email delivery](/automations/email).
  </Step>
  <Step title="Create">
...
<Steps>
  <Step title="Open run history">
    From an automation, click **Executions**, or open the report's run history. See [Run history](/automations/history).
  </Step>
