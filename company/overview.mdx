---
title: "Company details"
description: "Set your company information and account-level branding."
---

The **Company details** page lives at **Settings → Company**. It holds the account-level information about your company, along with the logos your reports can use. Editing this page requires **Admin access**.

The page has two sections: **Company information** and **Agency branding**.

## Company information

These fields describe your company across the account. Edit them and then save.

| Field | Description |
|---|---|
| **Company name** | Your company's name. Required. |
| **Website address** | Your company website URL. |
| **City** | The city your company is based in. |
| **State** | The state or region. |
| **Country** | The country, chosen from a list. |
| **Phone number** | A contact phone number. |
| **Timezone** | Used as the starting reporting timezone for new clients. |
| **Default currency** | Used as the starting currency for new clients. |

## Agency branding

The **Agency branding** section lets you upload logos that your reports can use instead of client-specific branding. See [Branding](/company/branding) for details on how the logos work and how client-level branding overrides them.
